...
Unity simulation environment for knml_rover
...
> You can skip this step if you use [`knml_robot`](https://github.com/knmlprz/knml_rover/tree/main#) since `unity_sim` is already included there as a submodule.

```bash
git clone git@github.com:knmlprz/knml_sim.git src/unity_sim
...
1. Instantiate Knml Prefab in the scene.
2. You can now drive on the terrain.
3. Instantiate the FollowCamera Prefab.
4. Configure the FollowCamera Component on FollowCamera object to follow the Knml object (to adjust the look-at pos, add an Empty GameObject as a child of Knml and follow that).
...
7. Use GPSProbe to find the world's yaw relative to true north and put that value as IMU's yaw offset in **Knml -> BaseLink -> IMU**.
8. Now `/imu/data` and `/gps/fix` messages will be published with the correct data. You should be able to see Knml's pose on the map in Ground Station.
